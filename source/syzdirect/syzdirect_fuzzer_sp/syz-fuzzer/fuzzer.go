...
type Fuzzer struct {
	...
	distanceFeature map[uint32]map[string]uint32 // distance -> feature -> # program in this distance with this feature
...
func main() {
	...
	fuzzer := &Fuzzer{
		...
		distanceFeature:          make(map[uint32]map[string]uint32),
		featureSequence:          make(map[string][]int),
	}
...
func (fuzzer *Fuzzer) updateDistanceFeature(dist uint32, feature string) {
	if dist != prog.InvalidDist {
		if _, ok := fuzzer.distanceFeature[dist]; !ok {
			fuzzer.distanceFeature[dist] = make(map[string]uint32)
		}
		fuzzer.distanceFeature[dist][feature] += 1
	}
}
func (fuzzer *Fuzzer) updateFeatureSequence(feature string, seq []int) {
	if _, ok := fuzzer.featureSequence[feature]; !ok {
		fuzzer.featureSequence[feature] = seq
	}
}
...
func (fuzzer *Fuzzer) addInputFromAnotherFuzzer(inp rpctype.Input) {
	...
	p.Dist = inp.Dist
	p.Feature = inp.Feature
	p.FeatureSequence = inp.FeatureSequence
...
func (fuzzer *FuzzerSnapshot) computeWeightedLCS(a, b []int) float64 {
	m, n := len(a), len(b)
	dp := make([][]int, m+1)
	for i := range dp {
		dp[i] = make([]int, n+1)
	...
	return float64(lcsLength)
}

func (fuzzer *FuzzerSnapshot) getdistfromfeature(feature string) uint32 {
	for dist, featureMap := range fuzzer.distanceFeature {
		for f, _ := range featureMap {
			if feature == f {
				return dist
			}
		}
...
func (fuzzer *FuzzerSnapshot) chooseProgram(r *rand.Rand, ct *prog.ChoiceTable) *prog.Prog {
	...
	// 计算特征全局出现频次
	// featureGlobalCount := make(map[string]uint32)
	featureCount := make(map[string]uint32)
	featureGlobalDistance := make(map[string]uint32)
	featureDistance := make(map[string]uint32)
	// simfeatureGlobalDistance := make(map[string]float64)
	for distance, featureMap := range fuzzer.distanceFeature {
		// featureGlobalCount[feature] += count //值越大说明，该特征在距离越近的程序中出现次数越多
		for feature, _ := range featureMap {
			featureCount[feature]++
			// 更新最小距离权重
			if weight, ok := featureDistance[feature]; !ok || distance < weight {
				featureDistance[feature] = distance
				featureGlobalDistance[feature] = distance
			}
		}
	}

	for feature, comcount := range featureCount { //遍历特征全局出现频次，获取没有完全匹配的特征
		if comcount > 2 {
			continue
		}
		if int(comcount) < len(fuzzer.featureSequence)/10 {
			// var simcount uint32
			// tarfeature := feature
			targetSeq := fuzzer.featureSequence[feature]
			// tardist := fuzzer.getdistfromfeature(tarfeature)
			if len(targetSeq) == 0 {
				featureGlobalDistance[feature] = uint32(0xffffffff)
				continue
			}
			// lenth := len(fuzzer.featureSequence)/10 - int(comcount)
			processor := NewFeatureProcessor(fuzzer.distanceFeature, fuzzer.featureSequence, r)
			targetSet := processor.createTargetSet(targetSeq)
			candidates, err := processor.processCandidates(featureDistance, targetSeq, targetSet)
			if err != nil {
				continue
			}
			simfeatureDistance := processor.calculateFinalWeight(candidates, targetSeq, featureDistance)
			if featureGlobalDistance[feature] > uint32(simfeatureDistance) {
				featureGlobalDistance[feature] = uint32(simfeatureDistance)
			}
		}
	}
	totalfeatureDistance := uint32(0)
	for _, distance := range featureGlobalDistance {
		if threeQuarterOfDistance > 0 {
			totalfeatureDistance += threeQuarterOfDistance - distance
		}
	}
	...
	featureGlobalWeight := make(map[string]uint32)
	prioMap := make(map[DistFeature]uint32)
	sumPrios := uint32(0)
	for distance, featureMap := range fuzzer.distanceFeature {
		if distance >= threeQuarterOfDistance {
			continue
		}
		distanceWeight := (threeQuarterOfDistance - distance) * 1000 / totalWeight
		// distanceCount := float64(fuzzer.distanceGroup[distance]) / float64(maxCount)
		// alpha := 1.0 * distanceCount
		// featureGlobalCount[feature] += count //值越大说明，该特征在距离越近的程序中出现次数越多
		for feature, count := range featureMap {
			featureGlobalWeight[feature] = (threeQuarterOfDistance - featureGlobalDistance[feature]) * 1000 / totalfeatureDistance
			combinedWeight := distanceWeight + featureGlobalWeight[feature]
			// combinedWeight := (1-alpha)*distanceWeight + alpha*float64(featureGlobalDistance[feature])
			prioMap[DistFeature{distance, feature}] = combinedWeight
			sumPrios += combinedWeight * uint32(count)
		}
	}
	randVal := uint32(r.Int63n(int64(sumPrios)))
	for _, p := range fuzzer.corpus {
		if p.Dist < threeQuarterOfDistance {
			currPrio := prioMap[DistFeature{p.Dist, p.Feature}]
			if currPrio > randVal {
				return p
			}
			randVal -= currPrio
...
func (fuzzer *Fuzzer) addInputToCorpus(p *prog.Prog, sign signal.Signal, sig hash.Sig) {
	...
	if _, ok := fuzzer.corpusHashes[sig]; !ok {
		...
		fuzzer.updateDistanceFeature(p.Dist, p.Feature)
		fuzzer.updateFeatureSequence(p.Feature, p.FeatureSequence)
		// prio := int64(1)
...
func (fuzzer *Fuzzer) snapshot() FuzzerSnapshot {
	...
	tmpFeature := make(map[uint32]map[string]uint32, len(fuzzer.distanceFeature))
	for distance, featureMap := range fuzzer.distanceFeature {
		tmpFeature[distance] = featureMap
	}
	tmpFeatureSequence := make(map[string][]int, len(fuzzer.featureSequence))
	for features, sequence := range fuzzer.featureSequence {
		tmpFeatureSequence[features] = sequence
	}
	fuzzer.corpusMu.RUnlock()
	return FuzzerSnapshot{fuzzer.corpus, tmpGroup, tmpFeature, tmpFeatureSequence}
}
